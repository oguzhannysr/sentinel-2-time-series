...
def ErrorMetrics(inputExcelPath, outputExcelPath):
    
    ExcelWorkbook = openpyxl.load_workbook(filename = inputExcelPath ,data_only=True)
    crop = 'Wheat'
    #%%
    ...
    if crop == 'Barley':
        usecols= 'A:AR'
        data = pd.read_excel(inputExcelPath,
                           sheet_name=crop, 
                           usecols=usecols, 
    ...
    veg_name_for = pd.read_excel(inputExcelPath,
                       sheet_name=crop, 
                       usecols= 'A')
    ...
    for k,l in zip(threshold_cor_range,threshold_mindist_range):
        ...
        while e < ws.max_row:
          ...
          #      detect_cor.loc[k/100][q] = detect_corelation.loc[q].values[0]
            ...
            with pd.ExcelWriter(outputExcelPath + "/" + "{}_{}_{}_{}.xlsx".format(veg_name_for[d],k,l,crop)) as writer1:
