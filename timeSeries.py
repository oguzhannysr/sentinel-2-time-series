...
def (safe_pathh, output_path, vectorfile_path):
    
    rasters_path_dir = []
    parcels = gpd.read_file(vectorfile_path)  



    for files in os.walk(safe_path):
    ...
    for i in range(0,len(diff)):
        ...
        globals()['d_' + '%s' % diff[i]]['_ROWID_'] = d_20201005['_ROWID_']  
        dates.append(diff[i])
    dates.sort()
    ...
    while b<=parcels.shape[0]*len(bands_vgind_name): 
        ...
        for i in range(0,len(dates)):
            ...
            if i == (len(dates)-1):
                ...
                while q < globals()['%s' % split[2]].shape[0]:
                    ...
                    q = q+ len(dates)
                    w = w + len(dates)
                #globals()['%s' % split[2]]["mean"] = globals()['%s' % split[2]]["mean"].ewm(span=5).mean()#0.25 0.5 0.75 #ewm

    #%%
    with pd.ExcelWriter(output_path + "_TimeSeries_{}.xlsx".format(datetime.datetime.today().date())) as writer:
